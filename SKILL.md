...
2. Read the current approval `files` mapping. Scan Markdown files under `evidence/` instead of deriving a path from an unchecked task ID. A dependency is complete only when one record names that task, has `status: verified`, and repeats the current approval mapping as `planning_revision`.
3. A task is ready when it has no current verified evidence and every dependency does. Use a requested ready task, or the first ready task in Task Graph order. Do not run a task whose dependencies are incomplete.
...
Create one non-overwriting Markdown record under `evidence/`. Choose a filesystem-safe filename independently of the task ID and confirm its resolved path remains inside `evidence/`. Include:
